...
:- doc(summary,
"The file iso_tests.pl has a set of assertions tests which have been
developed according to examples used in the ISO standard for PROLOG and
therefore, all the tests meet the standard.

The battery of tests contained in the file iso_tests.pl is based in the
application Stdprolog developed by the researchers Péter Szabó  and Péter
...
The Stdprolog application contains all the tests based in the examples of the
ISO standard and a set of tests obtained from Sictus and Eddbali. The most of
...
The tests from SICTUS and Eddbali have also been included becuase it is
interesting to do test beyond those established by the standard.").
").

:- doc(module,
"The battery of tests is organized in such way that the test for each one of
the predicates are grouped together but there is a separation between the ISO
standard tests and the SICTUS and Eddbali ones.
...
Where preconditions,postconditions and properties are optional.


Each one of the tests developed comes with a brief explanation, which I think
will  be useful to understand the relevance of the tests.
...
specific way in the execution of a predicate). This kind of error does not
happen in the new implementation; if a test fails is becauseit  does not
work as expected.


Once all of the tests have been implemented it is now time to analyze the reasons
why some of the tests do not succeed in Ciao.

1- The most common cause why  a test does not succeed in Ciao is because
the format of the error raised  does not match with the format specified by
the ISO standard for that error.
     ...
     Syntax error:
	    ...
	    Ciao format: error(sytax_error(Ln0,Ln1,Msg,Errorloc),Imp_dep).
     ...
     Type error:
	    ...
	    atomchars_test13, numberchars_test16 
	     ...
	     for the predicate call/1 (pag 44 standard)
		...
		type_error(callable,1) instead of type_error(callable,(write(3),1)).
	     ...
	     for the predicates asserta/1 and assertz/1 (pag 79 standard)
		...
		type_error(clause,(foo:-4)) instead of type_error(callable,(4)).
	     ...
	     for the predicate atom_chars/2 (pag 108 standard), when Atom is a
		...
		type_error(atom,f(b)) instead of type_error(character,f(b)).
	     ...
	     for the predicate number_chars/2 (pag 111 standard), when an
		...
		type_error(atom,2) instead of type_error(character,2).
	     ...
	     for the predicate get_char(S_or_a,Char) (pag 91 standard), when
		...
		type_error(atom,1) instead of type_error(in_character,1).
	     ...
	     for the predicate peek_char(S_or_a,Char) (pag 93 standard), when
	     	Char is neither a variable nor an in-character: 
	     	ISO standard format: type_error(in_character,Char).
	    	Ciao format: type_error(atom,Char).
     ...
     Permission error:
	    ...
	    peekbyte_test13,putbyte_test8,read_test15,read_test20,write_test20
	     ...
	     for the predicate clause/2 (pag 77 standard) when the predicate
	     ...
	     for the predicate set_input/1 (pag 87 standard) when the target
	     ...
	     for the predicate set_output/1 (pag 87 standard) when the target
	     ...
	     for the predicate flush_output/1 (pag 89 standard) when the
	     	target stream is an input stream.
	     	ISO standard format: error(permission_error(output,stream,S_or_a),Imp_dep)    
	    	Ciao format:error(permission_error(modify,stream,S_or_a),Imp_dep).


	     for the predicate get_char/2,get_code/2,peek_char/2,peek_code/2
	     	(pag 91 and 93 standard),when the target stream is an output stream:
	     	ISO standard format: error(permission_error(input,stream,S_or_a),Imp_dep)    
	    	Ciao format:error(permission_error(access,stream,S_or_a),Imp_dep).


	     for the predicate get_char/2,get_code/2,peek_code/2,peek_char2
	     ...
	     for the predicate put_char/2,put_code/2,nl/1 (pag 94 standard)
	     	when the target stream is an input stream.
	     	ISO standard format: error(permission_error(output,stream,S_or_a),Imp_dep)    
	    	Ciao format: error(permission_error(modify,stream,S_or_a),Imp_dep).

	     for the predicate get_byte/2 and peek_byte (pag 96 and 97
	     	standard, when the target stream is an output stream.
	     	ISO standard format: error(permission_error(input,stream,S_or_a),Imp_dep)    
	    	Ciao format:error(permission_error(access,stream,S_or_a),Imp_dep).


	     for the predicate get_byte/2 and peek_byte/2 (pag 96 and 97 standard),
	     ...
	     for the predicate put_byte/2 (pag 97 standard) when the target
	     	stream is a input stream.
	     	ISO standard format: error(permission_error(output,stream,S_or_a),Imp_dep)    
	    	Ciao format: error(permission_error(modify,stream,S_or_a),Imp_dep).


	     for the predicate read/2 (pag 99 standard) when the target stream
	     	is an output stream.
	     	ISO standard format: error(permission_error(input,stream,S_or_a),Imp_dep)    
	    	Ciao format: error(permission_error(access,stream,S_or_a),Imp_dep).
	     ...
	     for the predicate write/2 (pag 100 standard) when the target
	     	stream is an input stream.
	     	ISO standard format: error(permission_error(output,stream,S_or_a),Imp_dep)    
	    	Ciao format: error(permission_error(modify,stream,S_or_a),Imp_dep).
     ...
     Domain error:
	    ...
	    currentinput_test2, currentoutput_test2
	     ...
	     for the predicate current_input/2 (pag 86 standard) when the target
	     ...
	     for the predicate current_output/2 (pag 86 standard) when the target
	     	stream is neither a variable nor a stream.
	     	ISO standard format: error(domain_error(stream,Stream),Imp_dep)    
	    	Ciao format: error(domain_error(stream_or_alias,Stream),Imp_dep).


2- There are several tests in which an error is expected but in
...
3- The execution of a predicate raises a different error to the one specified
...
4- The execution of a predicate that according to the standard should succeed,
fails in Ciao. 
...
5- The execution of a predicate should raise an error but the execution of the
test succeeds.
...
6- Another causes of the failures of the tests in Ciao is that within
...
7- Ciao adds more information to a predicate created by the user than the one
that the standard considers.
     ...
     For example, the postcondition of the following tests is expecting that
     the variable Body is equal to insect(I) where insect(I) is a user
     defined predicate:
...
8- The execution of the predicate call_test7 in Ciao gets two results but
according to the ISO standard it should get just one.
	  ...
	  X=1,Z=1.
	  Therefore, looking at these two examples I think that there may be a problem
	  with the assignmnt of values to a variable in the predicate call.
	    

9- The execution of some tests does not succeed because there are some issues
related to the stream manipulation in the assertions tests:
	...
	- These issues provoke that the execution of some of the tests aborts
	     the execution of the whole battery of tests.
	     I have chosen to comment out these tests until this
	     issue is fixed because in other case it would be impossible to
	     execute the battery of tests.
	     ...
	     These tests are:	     		      
	      ...
	      - But if the test is expressed as followig the same problem is
	       not found and the test succeeds.
...
10- The standard does not establish what to do with those non-ASCII characters
but the Stdprolog developers added some tests from SICTUS and EDDBALI try to
...
11- There are some test that are ignored during their execution, this happens
because these tests abort the execution of Ciao. The tests are:
      - eval_test36



12- I have changed the expected result for some of the tests to accomodate
    them to Ciao. According to the standard when more than one error condition is
    satisfied, the error that is reported by the Prolog processor is
    implementation dependent. In some tests, Ciao raises a error different to the
    one specified by the standard but I have realised that the error that Ciao
    raises is due to another error condition that happens in the test. Therefore, I have
    changed the expected error of the test from the one specified by the
    standars to the one that Ciao raises.
    I think the most appropiate solution would be that the assertions test would
    allow specification of more than one expected error. Until this is completed I have
    chosen the solution explained above.
...
13- The predicate open/4 in Ciao does not have implemented the stream options
...
fail for several reasons:
	  ...
	  - There are side effects because open/4 does the same as open/3; for example, in the
	  preconditions of some tests a binary file or a file with a
	  determinate eof_action has to be opened in such a way
	  that the execution of the predicate that we are trying to check
	  should raise a specific error. As these stream options are not
	  implemented in Ciao the execution of the test does not behave as the
	  test is expected, and therefore the test fails. 
...
14- The predicate close/2 in Ciao does not have implemented the close options therefore,
close/2 does the same as close/1. This fact made that a multitude of test fail:
	  ...
	  adecuate. For example: close(Sc,_) succeeds in
	  Ciao.	
...
15- Other errors:
    ...
    - The execution of the predicates peek_char(S_or_a,Char) and get_char(S_or_a,Char), when stream
      position of the target stream is end_of_file it only succeeds if the atom
      end_of_file unifies with Char. It seems that in Ciao the end of a file
      is denoted by -1. This is correct,however the predicate peek_char is not
      able to recognize that -1 is equal to end_of_file. Therefore, if the predicates
      ...
      There are two thing that need to be said about it:
	    ...
	    - first of all,when Char is not a character, the error that should be raised is
	    representation_error(character),not
	    domain_error(character_code_list,[-1]); 
	    the one that Ciao raises.

	    - and Ciao should have a mechanism to equal end_of_file to -1
	    because if not either peek_char/2 or peek_code/2 are going to raise
	    one error when they attempt to read from a stream with position
	    end_of_stream.
...
16- Error: Malformed body:

    - The execution of the test cut_test10 provokes an error for malformed
      body (exactly in \+(\+(|))), but the syntax of this test is correct
      according to the standard. This test is included in the battery of
      tests but the test is commented out to avoid its execution.

    - The execution of the test ifthenelse_test9 provokes an error for malformed
      body but the syntax of this test is correct according to the
      standard. This test is included in the battery of tests but it is
      commented out to avoid its execution.

      I have chosen to comment out these tests because in other cases they
      abort the execution of the whole battery of tests.

    - The tests not_test2,not_test3 and not_test6 also provoke an error for
      malfomed body:
      ...
      In this case I have chosen to write these tests in the second way to
      avoid the problem of malformed body.

	

17-  the result of executing current_output(S) is S=user_output but when the
...
18- The test atomchars_test8 is ignored but the predicate that this test
executes succeeds in Ciao. ESTO PARA COMENTARLOS EN ISSUES OF ASSERTIONS TESTS.
	 atom_chars(_,_).




19 The execution of the test repeat_test1 is commented out because
  in Ciao there is no way yet to limit the execution of a predicate for a
  limited period of time, therefore it would be executing forever. A possible
  solution would be to establish a time-out for these predicate but until it
  is done I have left this test commeted.


20- Side effects of the execution of some tests:

    - The execution of the tests op_test18 and op_test19 should, according to
    the standard, raise an error but they succeed in Ciao. It provokes
    that the priority of the operator ',' be changed. Due to this the test
    current_op_test1 fails since this test checks that the priority of the
    operator ',' is the default established in the standard.

    - The result of executing the test getchar_test15 is modified
    because of the execution of the test getchar_test14. The test
    getchar_test14 executes the predicate get_char(1) where the target stream
    is user_input. This test fails because Char is not an in_character. The
    problem is that in the test getchar_test16, the predicate that is executed is
    get_char(user_input,1) the target stream is also user_input, after
    the execution of getchar_test15, has position past_end_of_stream. This
    provokes that the error raised by Ciao is a
    permission_error(access,past_end_of_stream,user_input) instead of 
    type_error(in_character,1) that is the expected error and the one that
    would be obtenied if the test getchar_test14 was not been executed before.

    -The same as the explained above happens with the tests getcode_test24
    getcode_test25 and getcode_26, where the result of getcode_test25 is modified
    because of the execution of getcode_test25, and with the tests
    peekchar_test15 and peekchar_tes16


21- There are some arity mismatch issues. 
	 ...
	 This is similar, but does not provoke the same error:
	  ...
	  not affect the execution of the other tests these two tests have
	  not been changed.
...
tests that allow:
      ...
      - Check the postconditions of a test even when it has failed. Sometimes
	the expected result for a test is failure. However, if a test fails it
	does not mean that the postconditions do not have to be checked. The postconditions allow to
	check that the changes carried out by a test are the changes that are expected, in such a
	way that even when the expected result of a test is failure and the test
	fails, this test should only be considered to have failed if it does not meet the
	postcondition established for it.
	Example: read_test4

      - It should be a good a idea to be able to specify more than one
        error for one test. Sometimes, in the execution of one predicate,
        it meets more than one error condition so it is an implementation
        dependent decision which one to raise. It is because of this reason I
        think it would be a good idea to allow specification of all the possible errors that a test
        might expect.
").
